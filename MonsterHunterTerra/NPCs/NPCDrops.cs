...
namespace MonsterHunterTerra.NPCs
{
    public class NPCDrops : GlobalNPC
    {
        public override void NPCLoot(NPC npc)
        {
            ...
            {
                if (!MonsterHunterTerraWorld.spawnOre)
                {                                                          //Red  Green Blue
                    Main.NewText("The world has been blessed with Machalite Ore", 200, 200, 55);  //this is the message that will appear when the npc is killed  , 200, 200, 55 is the text color
                    for (int k = 0; k < (int)((double)(WorldGen.rockLayer * Main.maxTilesY) * 40E-05); k++)   //40E-05 is how many veins ore is going to spawn , change 40 to a lover value if you want less vains ore or higher value for more veins ore
                    {
                        ...
                        WorldGen.OreRunner(X, Y, WorldGen.genRand.Next(9, 15), WorldGen.genRand.Next(5, 9), (ushort)mod.TileType("Ore1Tile"));   //WorldGen.genRand.Next(9, 15), WorldGen.genRand.Next(5, 9) is the vein ore sizes, so 9 to 15 blocks or 5 to 9 blocks, mod.TileType("CustomOreTile") is the custom tile that will spawn
                    }
                }
